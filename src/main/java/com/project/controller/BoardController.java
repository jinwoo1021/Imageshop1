...
public class BoardController {
	...
	//게시글 목록 페이지 (페이징 요청 정보를 매게 변수로 받고 다시 뷰에 전달한다.
	...
	//게시글 상세 페이지, 페이징 요청 정보를 매게 변수로 받고 다시 뷰에 전달한다.
	...
	public void modifyForm(int boardNo, @ModelAttribute("pgrq")
	PageRequest pageRequest,Model model)throws Exception {
		//조회한 게시글 상세 정보를 뷰에 전달한다.
		Board board = service.read(boardNo);
		
		model.addAttribute(board);
	}
	//게시글 수정처리 페이징 요청 정보를 매개변수를 받고 다시 뷰에 전달한다.
	...
	public String modify(Board board, PageRequest pageRequest,
	 ...
	 //게시글 삭제 처리,페이징 요청 정보를 매개변수로 받고 다시뷰에 전달한다.
	...
	public String remove(int boardNo, PageRequest pageRequest 
		...
		//RedirectAttribute 객체에 일회성 데이터를 전달한다.
		rttr.addAttribute("page", pageRequest.getPage());
		rttr.addAttribute("sizePerPage", pageRequest.getSizePerPage());
		rttr.addFlashAttribute("msg", "SUCCESS");
